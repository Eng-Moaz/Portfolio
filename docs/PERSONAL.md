...
Edit `src/content/pages/about.mdx` for all personal paragraphs, hobby headings, links, captions and photo groups. The Chess.com profile and ratings, HSK 3 progress, and project training-platform experience were supplied by Moaz on 20 September 2026; ratings are author-provided, not live synchronized. Remove or update that sentence as ratings change.

Store real photos under `public/images/about/`. Optimize photos before adding them; do not put private originals in public. Replace a Chinese photo slot by adding metadata like this (use your actual dimensions):

```yaml
media:
  - src: /images/about/chinese-study.webp
    group: chinese
...
At build time the shared layout checks whether that file exists. When present, every page displays a compact **Download CV** link beside the theme switch. The same-origin URL is `/files/Moaz_Mohamed_CV.pdf` and uses the HTML `download` attribute. If missing, no download link is rendered. Keep the filename unchanged for replacements.
